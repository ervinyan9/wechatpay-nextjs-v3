...
import { IcloseSubOrders, IcombineApp, IcombineH5, IcombineJsapi, IcombineNative } from './src/types-assemble';
import { BatchesTransfer, FindRefunds, ProfitSharing, Refunds, UploadImages } from './src/types-v2';
import { Base } from './src/Base';
import { IPayRequest } from './src/PayRequest';
import { FetchPayRequest } from './src/FetchPayRequest';
...
class Pay extends Base {
  ...
  public constructor(arg1: Ipay | string, mchid?: string, publicKey?: Buffer, privateKey?: Buffer, options?: Ioptions) {
    ...
    this.httpService = new FetchPayRequest();
  }
  ...
  public async get_certificates(apiSecret: string): Promise<ICertificates[]> {
    ...
    if (result.status === 200) {
      ...
      for (const item of data) {
        ...
        item.publicKey = forge.pki.publicKeyToPem(publicKey);
      }
  ...
  private async fetchCertificates(apiSecret?: string) {
    ...
    if (result.status === 200) {
      ...
      data.forEach((item) => {
        ...
        // 获取证书的公钥
        const publicKey = cert.publicKey as forge.pki.rsa.PublicKey;
        newCertificates[item.serial_no] = forge.pki.publicKeyToPem(publicKey);
      });
